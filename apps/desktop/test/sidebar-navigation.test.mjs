...
test("sidebar shows a bounded standalone session list before retained projects", () => {
  const projectsToolbar = sidebarSource.match(
    /data-sidebar-section="projects"[\s\S]*?<ProjectSourceMenu[\s\S]*?<\/div>/,
  )?.[0] ?? "";
  ...
  assert.match(projectsToolbar, /<ProjectSourceMenu/);
  // 新建项目 and 远程连接 are ProjectSourceMenu menu items, tested in its own suite.
  ...
  assert.ok(
    sidebarSource.indexOf('data-sidebar-session-section="temporary"') <
      sidebarSource.indexOf('<ProjectSourceMenu'),
  );
  assert.match(
    globalStyles,
