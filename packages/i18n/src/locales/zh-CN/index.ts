...
export const zhCN = {
    ...
    tray: {
        ...
        askBody: "关闭窗口后，PI-Desktop 可以继续在系统托盘中运行，避免丢失任何内容。你随时可以在设置中更改此选项。",
        closeToTray: "关闭到托盘",
        confirmQuitTitle: "确定退出 PI-Desktop？",
        confirmQuitBody: "退出将停止所有正在进行的会话，未保存的内容可能会丢失。你确定要退出吗？",
        confirmQuit: "退出",
    },
    pluginDesktopConsent: {
        message: "{name} 想要执行 {operation}",
        arguments: "参数：{args}",
        detail: "这是一个危险的桌面操作。插件已确认风险；是否执行由你决定。如果你没有预期到这个操作，请拒绝。",
        allowOnce: "允许一次",
        deny: "拒绝",
    },
    pluginFsConsent: {
    ...
    settings: {
        ...
        sklm: {
            ...
            dnsHint: "该校验在本机解析主机,而下载本身会走代理。如果你使用代理、VPN 或自定义 DNS,请检查 设置 → 通用 → 网络。",
            ...
            fakeIpHint: "代理把 {{host}} 应答为 {{address}} —— 这是代理用作 fake-IP 占位地址的保留基准测试段,不是该源的真实地址。请把代理切换为系统代理模式,或检查 设置 → 通用 → 网络。",
            fakeIpHintPlain: "本地代理把该主机解析成了保留段里的 fake-IP 占位地址,而不是该源的真实地址。请把代理切换为系统代理模式,或检查 设置 → 通用 → 网络。",
        ...
        pluginProviderManaged: "此服务来自插件“{{plugin}}”，接口地址和模型均由该插件提供。可在“扩展”页面启用或停用。",
        ...
        configSync: {
            ...
            allowInsecureHttp: "允许在受信任的内网地址使用 HTTP",
            allowInsecureHttpWarning: "HTTP 不会加密 WebDAV 凭据。仅对你控制的受信任内网地址启用；公网 HTTP 地址会被拒绝。",
            remoteMode: "服务器兼容模式",
        ...
        importAgentSkillsDesc: "查找本机 Claude、Codex 等工具的 skill 目录，勾选后导入到当前级别。",
        importAgentMcpTitle: "从其他工具扫描 MCP",
        importAgentMcpDesc: "查找 Claude Desktop、Cursor、Codex、Claude Code、opencode 的 MCP 配置，勾选后写入本机 MCP 列表。",
        ...
        importAgentScanModeHint: "复制：拷贝到本机后与源解耦；软链接：外部改动下次扫描自动生效，源被删除或移动会失效。仅对技能生效。",
        importAgentScanResult: "导入完成：成功 {{imported}}，跳过 {{skipped}}，失败 {{failed}}",
        ...
        importModelsScanDesc: "查找本机的提供商和模型设置。会复制已保存在配置中的 API 密钥，不会复制订阅登录。",
        ...
        marketCustomUrlDesc: "catalog.json 的完整地址。目录内的安装包相对路径会基于该地址解析。",
        ...
        largePasteThresholdDesc: "纯文本粘贴超过此字符数后，会转换为当前会话的临时文件。",
        ...
        promptEnhancementDesc: "作用于输入框的「增强提示词」操作。系统提示词为内置，可自定义用户模板与增强模型。",
        promptEnhancementCustomTemplate: "使用自定义提示词",
        promptEnhancementCustomTemplateDesc: "用你自己的用户模板替换内置模板。系统提示词保持内置。",
        promptEnhancementCustomTemplateNeedsTemplate: "请先保存自定义模板；保存后即可用开关在内置模板与自定义模板之间切换。",
        promptEnhancementEdit: "编辑",
        promptEnhancementModelTitle: "增强提示词",
        promptEnhancementModel: "默认模型",
        promptEnhancementThinking: "思考强度",
        promptEnhancementThinkingDesc: "改写时使用的思考强度。默认关闭，最快。",
        ...
        promptEnhancementUserTemplateDesc: "用于包裹草稿。必须包含草稿变量，请用插入按钮写入。",
        promptEnhancementInsertDraft: "插入草稿变量",
        promptEnhancementRestore: "恢复默认",
        promptEnhancementMissingDraftVariable: "用户模板必须包含草稿变量，否则草稿无法发送给模型。",
    ...
    remote: {
        ...
        online: "在线",
        offline: "离线",
        browseFailed: "无法列出主机上的该文件夹。",
        "cancel": "取消",
        noHosts: "还没有配对的远程主机。请先在 设置 → 远程主机 中添加，再回到这里连接。",

    terminal: {
      notRemoteTitle: "终端",
      notRemoteBody: "打开远程会话后即可使用远程 shell。",
      opening: "正在远程主机上打开 shell…",
      openFailed: "无法在这台主机上打开 shell。",
      exited: "shell 已退出。",
      retry: "重试",
    },},
    project: {
        ...
        createSourceRemote: "远程主机",
        openSourceMenu: "添加项目来源",
    ...
    plugins: {
        ...
        permissionHelp: {
            ...
            "fs.delete": "可删除声明范围内或自己写入的文件；删除进入系统回收站，其余路径每次都会先问你。",
            ...
            "desktop.control": "允许插件调用经过审查的 PI-Desktop 操作目录；破坏性操作仍需 confirm=true，插件永远拿不到 MCP bearer token。",
            ...
            "browser.cdp": "可导航工作面板浏览器、读取页面、运行 JavaScript，并发送白名单内的 Chrome DevTools 命令。Cookie 与存储相关方法会被拒绝。",
            "usage.read": "分页列出已完成回合的用量事实（每回合 token 计数与会话标题）。不包含任何消息内容。",
        },
    },
    extensions: {
        scope: {
        ...
        subagents: {
            ...
            toolsInheritHint: "会带上主会话可用的 Skill、MCP 和插件工具。Task、模式切换、Ask、ToolSearch 和 new_context 仍留在主会话。",
    ...
    status: {
        ...
        restored: "连接已恢复",
        fatal: "无法连接本地服务",
        unsupportedGlibc: "当前 Linux 构建需要 glibc 2.35 或更高版本（Ubuntu 22.04、Debian 12、Fedora 36+）。",
        dbSchemaTooNew: "当前 PI-Desktop 比本地数据更旧（数据库 schema {{found}}，此版本仅支持 {{supported}}）。请安装上次打开这些数据的更新版本，或更高版本。",
        archMismatch: "当前安装的是 {{buildArch}} 版本，而这台机器是 {{machineArch}}，需要通过转译运行，速度更慢。请改装 {{machineArch}} 版本。",
    ...
    errors: {
        ...
        MUTATION_RETRY_BUDGET_EXHAUSTED: "同一处修改连续失败三次，本轮已停止，不再盲目重试。再说一次即可继续。",
        CONTEXT_TOO_LARGE: "上下文恢复后对话仍然过长。请缩短消息内容或新开对话。",
        CONTEXT_COMPACTION_FAILED: "无法压缩当前对话的模型上下文。",
        SUBAGENT_CONTEXT_OVERFLOW: "子智能体的任务超出了其模型的上下文上限。请缩小任务范围、为它选择上下文窗口更大的模型，或让它一次读取更少的内容。",
